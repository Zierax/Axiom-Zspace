...
the Python reference) instead of the Python validator (`python` remains default for reference, `c99` **recommended for batch** `46 ms/TIC` light `604×`):

```bash
python run_pipeline.py --synthetic --engine c99   # recommended for batch (604x)
