...
### 4.1b C99 sovereign engine

The discovery card can be computed by the **C99 sovereign engine**
(dependency-free C99 binary, kernels machine-generated by
[Purce](https://github.com/Zierax/Purce) and differentially verified against
the Python reference) instead of the Python validator (`python` remains default for reference, `c99` **recommended for batch** `42.8 ms/TIC` light `650×`):

```bash
python run_pipeline.py --synthetic --engine c99   # recommended for batch (650x)
python run_pipeline.py --tic 260128333 --engine c99
```

The card gains a `c99_sovereign_card` section with the C99 verdict; the rest
of the pipeline (ingestion → BLS → auditors → CVS) is unchanged. Windows
builds require WSL (gcc). Full details, verification commands and the CLI
contract: [`docs/C99_ENGINE.md`](C99_ENGINE.md).
...
--engine {python,c99}  sovereign engine: python reference (default) or c99 binary
...
# controlled with the C99 sovereign engine (same sample, same seeds)
